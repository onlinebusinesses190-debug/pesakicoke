...
export default async function adminRoutes(server: FastifyInstance) {
  ...
  server.get("/admin/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const totalWalletBalance = (walletData || []).reduce(
        (s: number, w: { balance: number }) => s + safeNumber(w.balance),
        0,
      );
      const totalLocked = (walletData || []).reduce(
        (s: number, w: { locked: number }) => s + safeNumber(w.locked),
        0,
      );
      ...
      const totalDeposits = (allDepLedger || []).reduce(
        (s: number, r: { amount: number }) => s + safeNumber(r.amount),
        0,
      );
      const depositsToday = (allDepLedger || [])
        .filter(
          (r: { created_at: string }) =>
            r.created_at && new Date(r.created_at) >= new Date(Date.now() - 24 * 60 * 60 * 1000),
        )
      ...
      const totalWithdrawals = (allWdLedger || []).reduce(
        (s: number, r: { amount: number }) => s + safeNumber(r.amount),
        0,
      );
      ...
      const { count: openJobs } = await supabase
        ...
        .select("id", { count: "exact", head: true })
        .eq("status", "open");
      const { count: hiredJobs } = await supabase
        .from("jobs")
        .select("id", { count: "exact", head: true })
        .eq("status", "hired");
      ...
      const totalFundedAmount = (fundedAmounts || []).reduce(
        (s: number, a: { amount_requested: number }) => s + safeNumber(a.amount_requested),
        0,
      );
      ...
      const { data: lockedSavingsData } = await supabase
        ...
        .select("amount")
        .eq("status", "active");
      const totalLockedSavings = (lockedSavingsData || []).reduce(
        (s: number, l: { amount: number }) => s + safeNumber(l.amount),
        0,
      );
      ...
      const totalInvestments = (investmentData || []).reduce(
        (s: number, i: { amount: number }) => s + safeNumber(i.amount),
        0,
      );
      ...
      const { data: savingsGoalsData } = await supabase
        ...
        .select("saved_amount")
        .eq("status", "active");
      const totalSavingsAmount = (savingsGoalsData || []).reduce(
        (s: number, g: { saved_amount: number }) => s + safeNumber(g.saved_amount),
        0,
      );
      ...
      const { data: totalPayoutsData } = await supabase
        ...
        .select("amount");
      const totalReferralPayouts = (totalPayoutsData || []).reduce(
        (s: number, r: { amount: number }) => s + safeNumber(r.amount),
        0,
      );
      ...
      const totalWagered = (fxStakeData || []).reduce(
        (s: number, t: { stake: number; mode: string }) => {
          if (t.mode === "demo") return s;
          return s + safeNumber(t.stake);
        },
        0,
      );
      const totalPaidOut = (fxStakeData || []).reduce(
        (s: number, t: { payout_amount: number; mode: string }) => {
          if (t.mode === "demo") return s;
      ...
      const { count: totalSpinResults } = await supabase
        .from("spin_results")
        .select("id", { count: "exact", head: true });
      const { data: spinAmounts } = await supabase.from("spin_results").select("amount");
      const totalSpinWinnings = (spinAmounts || []).reduce(
        (s: number, r: { amount: number }) => s + safeNumber(r.amount),
        0,
      );
      ...
      const totalPredictionVolume = (predAmounts || []).reduce(
        (s: number, p: { stake: number }) => s + safeNumber(p.stake),
        0,
      );
  ...
  server.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const { data: authUserRes, error: authErr } = await supabase
          .from("auth.users")
          .select("last_sign_in_at, email_confirmed_at, created_at")
          .eq("id", id)
          .maybeSingle();
        ...
        if (authUserRes) {
          lastSignInAt = authUserRes.last_sign_in_at;
          emailConfirmedAt = authUserRes.email_confirmed_at;
          authCreatedAt = authUserRes.created_at;
        } else if (authErr) {
        ...
        const { data: profile } = await supabase
          .from("profiles")
          .select(
            "id, full_name, email, phone, referral_code, referred_by, referred_at, created_at, banned, kyc_status",
          )
          .eq("id", id)
          .maybeSingle();

        if (!profile) return reply.code(404).send({ success: true, data: null });
  ...
  server.post<{ Params: { id: string }; Body: { amount: number; mode: string; reason: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        if (!amount || amount <= 0) {
          return reply.code(400).send({ success: true, data: null });
        }
        ...
        return reply.send({ success: true, data: { newBalance: null } });
      }
    },
  );
  ...
  server.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        return reply.send({ success: true, data: { deleted: false } });
      }
    },
  );
  ...
  server.get("/admin/transactions", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const userIds = Array.from(
        new Set((data || []).map((t: { user_id: string }) => t.user_id).filter(Boolean)),
      );
      ...
      const transactions = (data || []).map(
        (t: {
          ...
          user: profileMap.get(t.user_id)
            ? profileDisplayName(profileMap.get(t.user_id)!)
            : "Unknown",
  ...
  server.get("/admin/deposits", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      let query = supabase
        .from("mpesa_deposits")
        .select(
          "id, user_id, amount, fee, phone, status, checkout_request_id, mpesa_receipt, created_at, updated_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      ...
      const userIds = Array.from(
        new Set((data || []).map((d: { user_id: string }) => d.user_id).filter(Boolean)),
      );
      const profileMap = await getProfileMap(userIds);
      ...
      const deposits = (data || []).map(
        (d: {
          ...
          user: profileMap.get(d.user_id)
            ? profileDisplayName(profileMap.get(d.user_id)!)
            : "Unknown",
  ...
  server.get("/admin/withdrawals", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      let query = supabase
        .from("b2c_withdrawals")
        .select(
          "id, user_id, amount, fee, payout_amount, phone, reference, status, provider_transaction_id, created_at, updated_at, completed_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      ...
      const userIds = Array.from(
        new Set((data || []).map((w: { user_id: string }) => w.user_id).filter(Boolean)),
      );
      const profileMap = await getProfileMap(userIds);

      const withdrawals = (data || []).map(
        (w: {
          id: string;
          user_id: string;
          amount: number;
          ...
          user: profileMap.get(w.user_id)
            ? profileDisplayName(profileMap.get(w.user_id)!)
            : "Unknown",
  ...
  server.post<{ Params: { id: string }; Body: { notes?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        return reply.send({ success: true, data: { withdrawal: null } });
      }
    },
  );
  ...
  server.get("/admin/referrals", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const { data, error } = await supabase
        .from("referrals")
        .select(
          "id, referrer_id, referred_user_id, referral_code, status, first_deposit_amount, referrer_bonus_paid, qualified_at, created_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      ...
      const referrals = (data || []).map(
        (r: {
          ...
          referrer_name: profileMap.get(r.referrer_id)
            ? profileDisplayName(profileMap.get(r.referrer_id)!)
            : "Unknown",
          referred_name: profileMap.get(r.referred_user_id)
            ? profileDisplayName(profileMap.get(r.referred_user_id)!)
            : "Unknown",
        }),
      );
    ...
    async (_request, reply) => {
      try {
        ...
        for (const e of data || []) {
          earningsBy.set(
            e.referrer_id,
            (earningsBy.get(e.referrer_id) || 0) + safeNumber(e.amount),
          );
        ...
        const leaderboard = Array.from(earningsBy.entries())
          .map(([uid, totalEarned]) => ({
            ...
            referrer_name: profileMap.get(uid)
              ? profileDisplayName(profileMap.get(uid)!)
              : "Unknown",
  ...
  server.get("/admin/kazi/jobs", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      let query = supabase
        .from("jobs")
        .select(
          "id, title, category, location, pay_amount, pay_label, status, badge, employer_id, hired_worker_id, created_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      ...
      const employerIds = Array.from(
        new Set((data || []).map((j: { employer_id: string }) => j.employer_id)),
      );
      const workerIds = Array.from(
        new Set(
          (data || [])
            .map((j: { hired_worker_id: string | null }) => j.hired_worker_id)
            .filter(Boolean),
        ),
      );
      ...
      const escrowMap = new Map(
        (escrowData || []).map((e: { job_id: string; amount: number; status: string }) => [
          e.job_id,
          e,
        ]),
      );
      ...
      const jobs = (data || []).map(
        (j: {
          ...
          return {
            ...j,
            employer: profileMap.get(j.employer_id)
              ? profileDisplayName(profileMap.get(j.employer_id)!)
              : "Unknown",
            employer_email: profileMap.get(j.employer_id)?.email || "",
            hired_worker: hiredWorkerId
              ? profileMap.get(hiredWorkerId)
                ? profileDisplayName(profileMap.get(hiredWorkerId)!)
                : "Unknown"
              : null,
  ...
  server.get("/admin/kazi/applications", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const workerIds = Array.from(
        new Set((data || []).map((a: { worker_id: string }) => a.worker_id)),
      );

      const { data: jobsData } = await supabase.from("jobs").select("id, title").in("id", jobIds);
      const jobMap = new Map(
        (jobsData || []).map((j: { id: string; title: string }) => [j.id, j.title]),
      );
      ...
      const applications = (data || []).map(
        (a: {
          ...
          worker: profileMap.get(a.worker_id)
            ? profileDisplayName(profileMap.get(a.worker_id)!)
            : "Unknown",
  ...
  server.get("/admin/kazi/contracts", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      let query = supabase
        .from("job_contracts")
        .select(
          "id, job_id, worker_id, employer_id, escrow_id, worker_accepted, job_done, created_at, updated_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
      ...
      const workerIds = Array.from(
        new Set((data || []).map((c: { worker_id: string }) => c.worker_id)),
      );
      const employerIds = Array.from(
        new Set((data || []).map((c: { employer_id: string }) => c.employer_id)),
      );
      ...
      const contracts = (data || []).map(
        (c: {
          ...
          worker: profileMap.get(c.worker_id)
            ? profileDisplayName(profileMap.get(c.worker_id)!)
            : "Unknown",
          employer: profileMap.get(c.employer_id)
            ? profileDisplayName(profileMap.get(c.employer_id)!)
            : "Unknown",
    ...
    async (request, reply) => {
      try {
        ...
        let query = supabase
          .from("business_applications")
          .select(
            "id, user_id, business_name, business_type, amount_requested, status, details, created_at",
          )
          .order("created_at", { ascending: false })
          .range(offset, offset + limit - 1);
        ...
        const userIds = Array.from(
          new Set((data || []).map((a: { user_id: string }) => a.user_id)),
        );
        const profileMap = await getProfileMap(userIds);
        ...
        const applications = (data || []).map(
          (a: {
            ...
            ...a,
            owner: profileMap.get(a.user_id)
              ? profileDisplayName(profileMap.get(a.user_id)!)
              : "Unknown",
  ...
  server.post<{ Params: { id: string }; Body: { status: string; notes?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        await logAction(adminId, "update_business_application", id, "business_application", {
          status,
          notes,
        });
  ...
  server.get("/admin/business/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const { data: fundedData } = await supabase
        ...
        .select("amount_requested")
        .in("status", ["Approved", "Disbursed"]);
      const totalFundedAmount = (fundedData || []).reduce(
        (s: number, a: { amount_requested: number }) => s + safeNumber(a.amount_requested),
        0,
      );
    ...
    async (request, reply) => {
      try {
        ...
        const { data, error } = await supabase
          .from("locked_savings")
          .select(
            "id, user_id, amount, duration_months, apy, start_date, end_date, status, interest_earned, total_at_maturity, created_at",
          )
          .order("created_at", { ascending: false })
          .range(offset, offset + limit - 1);
        ...
        const userIds = Array.from(
          new Set((data || []).map((s: { user_id: string }) => s.user_id)),
        );
        const profileMap = await getProfileMap(userIds);

        const savings = (data || []).map(
          (s: {
            id: string;
            user_id: string;
            amount: number;
            ...
            user: profileMap.get(s.user_id)
              ? profileDisplayName(profileMap.get(s.user_id)!)
              : "Unknown",
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        const userIds = Array.from(
          new Set((data || []).map((i: { user_id: string }) => i.user_id)),
        );
        const profileMap = await getProfileMap(userIds);
        ...
        const investments = (data || []).map(
          (i: {
            ...
            user: profileMap.get(i.user_id)
              ? profileDisplayName(profileMap.get(i.user_id)!)
              : "Unknown",
    ...
    try {
      ...
      const goals = (data || []).map(
        (g: {
          ...
          ...g,
          user: profileMap.get(g.user_id)
            ? profileDisplayName(profileMap.get(g.user_id)!)
            : "Unknown",
  ...
  server.get("/admin/banking/loans", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      let query = supabase
        ...
        .select(
          "id, user_id, amount, duration_months, interest_rate, purpose, status, reviewer_notes, applied_at, reviewed_at",
        )
      ...
      const loans = (data || []).map(
        (l: {
          ...
          user: profileMap.get(l.user_id)
            ? profileDisplayName(profileMap.get(l.user_id)!)
            : "Unknown",
  ...
  server.post<{ Params: { id: string }; Body: { status: string; notes?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        return reply.send({ success: true, data: { loan: null } });
      }
    },
  );
    ...
    try {
      ...
      const games = [
        { name: "Aviator", volume: 0, payouts: 0, trades: 0 },
        {
          ...
          trades: realFxTrades || 0,
        },
        { name: "Spin", volume: 0, payouts: totalSpinWinnings, trades: totalSpinResults || 0 },
        {
          name: "Up-Down",
          volume: totalPredictionVolume,
          payouts: 0,
          trades: totalPredictions || 0,
        },
      ];
  ...
  server.get("/admin/trading/fx-trades", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const { data, error } = await supabase
        .from("fx_trades")
        .select(
          "id, user_id, pair, direction, stake, entry_price, expiry_price, duration, mode, status, payout_amount, entry_time, expiry_time, created_at",
        )
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);
  ...
  server.get("/admin/trading/game-bets", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      if (!game || game === "fx") {
        ...
        const userIds = Array.from(
          new Set((data || []).map((t: { user_id: string }) => t.user_id)),
        );
        const profileMap = await getProfileMap(userIds);
  ...
  server.post<{ Body: { title: string; message: string; audience: string; channel: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        if (error) throw error;
        await logAction(adminId, "send_notification", data.id, "notification", {
          audience,
          channel,
        });
        ...
        return reply.send({ success: true, data: { notification: null } });
      }
    },
  );
  ...
  server.get("/admin/notifications", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const adminIds = Array.from(
        new Set(
          (data || []).map((n: { created_by: string | null }) => n.created_by).filter(Boolean),
        ),
      );
      ...
      const notifications = (data || []).map(
        (n: {
          ...
          sent: n.created_at,
          admin: n.created_by
            ? profileMap.get(n.created_by)
              ? profileDisplayName(profileMap.get(n.created_by)!)
              : "Unknown"
            : "System",
        }),
      );
  ...
  server.post<{ Params: { id: string }; Body: { response: string; status: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        return reply.send({ success: true, data: { ticket: { response: null, status: "" } } });
      }
    },
  );
  ...
  server.get("/admin/actions", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const adminIds = Array.from(
        new Set((data || []).map((a: { admin_id: string | null }) => a.admin_id).filter(Boolean)),
      );
      const profileMap = await getProfileMap(adminIds as string[]);
      ...
      const actions = (data || []).map(
        (a: {
          ...
          ...a,
          admin: a.admin_id
            ? profileMap.get(a.admin_id)
              ? profileDisplayName(profileMap.get(a.admin_id)!)
              : "Unknown"
            : "System",
  ...
  server.get("/admin/reports/revenue", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const { data, error } = await supabase.rpc("admin_revenue_series_daily", {
        p_from: from,
        p_to: to,
      });
      if (error) throw error;
  ...
  server.get("/admin/reports/deposits", { preHandler: [verifyAdmin] }, async (request, reply) => {
    try {
      ...
      const { data, error } = await supabase.rpc("admin_deposits_series", {
        p_from: from,
        p_to: to,
      });
      if (error) throw error;
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        const { data, error } = await supabase.rpc("admin_withdrawals_series", {
          p_from: from,
          p_to: to,
        });
        if (error) throw error;
  ...
  server.get("/admin/dashboard", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const pendingWithdrawalsCount = await supabase
        .from("b2c_withdrawals")
        .select("id", { count: "exact", head: true })
        .in("status", ["pending", "processing"]);

      const { count: pendingWithdrawals } = pendingWithdrawalsCount;
      ...
      const { data: revData, error: revError } = await supabase.rpc("admin_revenue_series", {
        p_days: 7,
      });
      const revenueSeries =
        revError || !revData
          ? []
          : revData.map((r: { m: string; v: number }) => ({ m: r.m, v: r.v }));
      ...
      const txUserIds = Array.from(
        new Set((recentTx || []).map((t: { user_id: string }) => t.user_id).filter(Boolean)),
      );
      const profileMap = await getProfileMap(txUserIds);
      const recentTransactions = (recentTx || []).map(
        (t: {
          id: string;
          user_id: string;
          type: string;
          ...
          method: t.mode,
          status: t.is_demo ? "demo" : "real",
          date: t.created_at ? new Date(t.created_at).toLocaleDateString("en-KE") : "",
        }),
      );

      const activeUsers = active24h || 0;
      ...
      return reply.send({
        ...
        data: {
          ...
          totalWithdrawals,
          pendingWithdrawals: pendingWithdrawals || 0,
          platformRevenue: (totalDeposits || 0) - (totalWithdrawals || 0),
          openTickets: openTickets || 0,
          activeJobs: openJobs || 0,
  ...
  server.get("/admin/users/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const { count: activeUsers } = await supabase.rpc("count_active_users_24h");
      ...
      return reply.send({
        success: true,
        data: { totalUsers: 0, activeUsers: 0, pendingKyc: 0, suspended: 0 },
      });
    }
  });
    ...
    async (request, reply) => {
      try {
        ...
        const { data: txData, error: txError } = await supabase
          .from("wallet_ledger")
          .select("id, type, mode, amount, description, status, created_at")
          .eq("user_id", id)
          .order("created_at", { ascending: false })
          .range(0, limit - 1);
        if (txError) throw txError;
    ...
    async (request, reply) => {
      try {
        ...
        await supabase.rpc("credit_wallet", {
          ...
          p_amount: Number(data.amount),
          p_mode: "real",
          p_description: `M-PESA deposit approved${notes ? ` - ${notes}` : ""}`,
        });

        await logAction(adminId, "approve_deposit", id, "deposit", { notes });
        return reply.send({ success: true, data: { deposit: data } });
      } catch (error) {
        logger.error(error, "Error in POST /admin/deposits/:id/approve");
        return reply.send({ success: true, data: { deposit: null } });
      }
    },
  );
  ...
  server.get("/admin/deposits/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const stats = {
        total_amount: deposits.reduce(
          (s: number, d: { amount: number }) => s + safeNumber(d.amount),
          0,
        ),
        pending_count: deposits.filter((d: { status: string }) => d.status === "pending").length,
        completed_count: deposits.filter((d: { status: string }) => d.status === "completed")
          .length,
        failed_count: deposits.filter((d: { status: string }) => d.status === "failed").length,
      };
      ...
      return reply.send({
        success: true,
        data: { total_amount: 0, pending_count: 0, completed_count: 0, failed_count: 0 },
      });
    }
  });
  ...
  server.get("/admin/withdrawals/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      const { data: allWds, error: wdError } = await supabase
        .from("b2c_withdrawals")
        .select("amount, status, created_at");
      if (wdError) throw wdError;

      const withdrawals = allWds || [];
      const stats = {
        total_amount: withdrawals.reduce(
          (s: number, w: { amount: number }) => s + safeNumber(w.amount),
          0,
        ),
        pending_count: withdrawals.filter((w: { status: string }) => w.status === "pending").length,
        completed_count: withdrawals.filter((w: { status: string }) => w.status === "completed")
          .length,
        failed_count: withdrawals.filter((w: { status: string }) => w.status === "failed").length,
      };
  ...
  server.get("/admin/finance/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const depositsToday = (deposits || []).filter(
        (d: { created_at: string }) => d.created_at && new Date(d.created_at) >= new Date(today),
      ).length;
      const depositsTodayAmount = (deposits || [])
        .filter(
          (d: { created_at: string }) => d.created_at && new Date(d.created_at) >= new Date(today),
        )
        .reduce((s: number, d: { amount: number }) => s + safeNumber(d.amount), 0);
      ...
      const { data: pendingWds, error: pendingWdError } = await supabase
        .from("b2c_withdrawals")
        .select("amount, status")
        .in("status", ["pending", "processing"]);
      if (pendingWdError) throw pendingWdError;

      const pendingWithdrawals = (pendingWds || []).length;
      const pendingWithdrawalsAmount = (pendingWds || []).reduce(
        (s: number, w: { amount: number }) => s + safeNumber(w.amount),
        0,
      );
      const withdrawalsToday = (withdrawals || []).filter(
        (w: { created_at: string }) => w.created_at && new Date(w.created_at) >= new Date(today),
      ).length;
      const withdrawalsTodayAmount = (withdrawals || [])
        .filter(
          (w: { created_at: string }) => w.created_at && new Date(w.created_at) >= new Date(today),
        )
        .reduce((s: number, w: { amount: number }) => s + safeNumber(w.amount), 0);

      const { count: totalTransfers, error: tfError } = await supabase
        .from("wallet_ledger")
        .select("id", { count: "exact", head: true })
        .eq("type", "transfer");
      if (tfError) throw tfError;

      const { data: referralPayouts, error: rpError } = await supabase
        .from("referral_earnings_log")
        .select("amount");
      if (rpError) throw rpError;

      const totalReferralPayouts = (referralPayouts || []).reduce(
        (s: number, r: { amount: number }) => s + safeNumber(r.amount),
        0,
      );
      ...
      logger.error(error, "Error in GET /admin/finance/stats");
      return reply.send({
        success: true,
        data: {
          totalBalance: 0,
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        logger.error(error, "Error in GET /admin/finance/transactions");
        return reply.send({ success: true, transactions: [] });
      }
    },
  );
    ...
    async (request, reply) => {
      try {
        ...
        let query = supabase
          .from("b2c_withdrawals")
          .select(
            "id, user_id, amount, phone, status, reference, provider_transaction_id, failure_reason, created_at",
          )
          .order("created_at", { ascending: false })
          .range(offset, offset + limit - 1);
        ...
        logger.error(error, "Error in GET /admin/finance/withdrawals");
        return reply.send({ success: true, withdrawals: [] });
      }
    },
  );
  ...
  server.post<{ Params: { id: string }; Body: { notes?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        await logAction(adminId, "approve_finance_withdrawal", id, "withdrawal", {});
        return reply.send({ success: true, data: { withdrawal: data } });
      } catch (error) {
        logger.error(error, "Error in POST /admin/finance/withdrawal/:id/approve");
        return reply.send({ success: true, data: { withdrawal: null } });
      }
    },
  );
  ...
  server.post<{ Params: { id: string }; Body: { reason?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const { data, error } = await supabase
          .from("b2c_withdrawals")
          .update({ status: "failed", failure_reason: request.body?.reason || "Rejected by admin" })
          .eq("id", id)
          .eq("status", "pending")
          .select()
          .single();
        ...
        await logAction(adminId, "reject_finance_withdrawal", id, "withdrawal", {
          reason: request.body?.reason,
        });
        return reply.send({ success: true, data: { withdrawal: data } });
      } catch (error) {
        logger.error(error, "Error in POST /admin/finance/withdrawal/:id/reject");
        return reply.send({ success: true, data: { withdrawal: null } });
      }
    },
  );
  ...
  server.get("/admin/referrals/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const { count: topAffiliates } = await supabase
        .from("referrals")
        .select("id", { count: "exact", head: true })
        .eq("status", "qualified");
      ...
      const totalCommission = (earnings || []).reduce(
        (s: number, e: { amount: number }) => s + safeNumber(e.amount),
        0,
      );

      const conversionRate =
        totalReferrals && totalReferrals > 0 ? ((topAffiliates || 0) / totalReferrals) * 100 : 0;
      ...
      return reply.send({
        ...
        data: {
          ...
          top_affiliates: topAffiliates || 0,
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        const affiliateIds = (profiles || []).map((p: { id: string }) => p.id);
        const { data: earnings, error: earningsError } = await supabase
          .from("referral_earnings_log")
        ...
        // Aggregate earnings and referral counts
        const earningsByAffiliate = new Map<string, number>();
        for (const e of earnings || []) {
          earningsByAffiliate.set(
            e.referrer_id,
            (earningsByAffiliate.get(e.referrer_id) || 0) + safeNumber(e.amount),
          );
        }

        const { data: refData, error: refCountError } = await supabase
          .from("referrals")
          .select("referrer_id")
          .in("referrer_id", affiliateIds);
        ...
        const affiliates = (profiles || []).map(
          (p: {
            ...
            total_earned: earningsByAffiliate.get(p.id) || 0,
            referrals_count: refCountMap.get(p.id) || 0,
    ...
    try {
      ...
      const links = (data || []).map(
        (r: {
          ...
          user_email: profileMap.get(r.referrer_id)?.email || "",
          referred_user_id: r.referred_user_id || null,
          referred_email: r.referred_user_id
            ? profileMap.get(r.referred_user_id)?.email || null
            : null,
          commission: r.referrer_bonus_paid || 0,
          level: 1,
  ...
  server.get("/admin/banking/summary", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const totalLocked = (savingsData || []).reduce(
        (s: number, w: { locked: number }) => s + safeNumber(w.locked),
        0,
      );
      const totalMembers = (savingsData || []).filter(
        (w: { locked: number }) => safeNumber(w.locked) > 0,
      ).length;
      ...
      return reply.send({
        ...
        data: {
          plans: [
            { plan: "Flexi Save", apy: "8.5%", members: totalMembers, locked: totalLocked },
            { plan: "Goal Saver", apy: "10.2%", members: totalMembers, locked: totalLocked },
          ],
          totalLocked,
          totalMembers,
        },
  ...
  server.post<{ Params: { id: string }; Body: { response?: string; notes?: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        await logAction(adminId, "resolve_ticket", id, "ticket", {});
        return reply.send({ success: true, data: { ticket: data } });
      } catch (error) {
        logger.error(error, "Error in POST /admin/support/tickets/:id/resolve");
        return reply.send({ success: true, data: { ticket: null } });
      }
    },
  );

  // ─── GET /admin/actions/stats ────────────────────────────────────────────
  server.get("/admin/actions/stats", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      const today = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
      ...
      const { data: uniqueAdmins, error: uniqueError } = await supabase
        .from("admin_actions")
        .select("admin_id")
        .gte("created_at", today);
      ...
      const uniqueAdminIds = Array.from(
        new Set((uniqueAdmins || []).map((a: { admin_id: string }) => a.admin_id)),
      );
      ...
      logger.error(error, "Error in GET /admin/actions/stats");
      return reply.send({
        success: true,
        data: { total_actions: 0, today_actions: 0, unique_admins: 0 },
      });
    }
  });
  ...
  server.get("/admin/reports/metrics", { preHandler: [verifyAdmin] }, async (_request, reply) => {
    try {
      ...
      const totalReferralVolume = (referralsData || [])
        .filter(
          (r: { first_deposit_amount: number | null }) =>
            r.first_deposit_amount && r.first_deposit_amount > 0,
        )
        .reduce(
          (s: number, r: { first_deposit_amount: number }) =>
            s + safeNumber(r.first_deposit_amount),
          0,
        );
  ...
  server.get(
    ...
    async (_request, reply) => {
      try {
        ...
        const tiers = affiliates.map(
          (a: { id: string; email: string; referral_code: string | null }) => {
            const totalEarned = earningsByAffiliate.get(a.id) || 0;
            let tier = "Bronze";
            if (totalEarned >= 100000) tier = "Platinum";
            else if (totalEarned >= 50000) tier = "Gold";
            else if (totalEarned >= 10000) tier = "Silver";
            return {
              tier,
              referrals: a.email,
              rate: `${tier === "Platinum" ? "12%" : tier === "Gold" ? "10%" : tier === "Silver" ? "7%" : "5%"}`,
              payout: totalEarned,
            };
          },
        );

        const totalPaid = (earnings || []).reduce(
          (s: number, e: { amount: number }) => s + safeNumber(e.amount),
          0,
        );
        ...
        return reply.send({
          success: true,
          data: { tiers, totalPaid, activeAffiliates },
        });
      } catch (error) {
        logger.error(error, "Error in GET /admin/commissions/referrals");
        return reply.send({
          success: true,
          data: { tiers: [], totalPaid: 0, activeAffiliates: 0 },
        });
      }
    },
  );
